 ...
 */
export class HeadCommand implements Command {
  name = "head";
  aliases = [];
  category = CommandCategory.TEXT_PROCESSING;
  description = "Output the first part of files";
  usage = "head [options] [file...]";
  examples = ["head file.txt", "head -n 5 file.txt", "head -20 file.txt"];
  ...
  async execute(
    ...
    // In a real terminal, head would read from stdin
    ...
    try {
      ...
      for (let i = 0; i < filePaths.length; i++) {
        ...
        // Add newline if file doesn't end with one and we're not at the last file
        if (i < filePaths.length - 1 && headLines.length > 0) {
          output += "\n";
        }
      }
